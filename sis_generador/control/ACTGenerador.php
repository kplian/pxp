    ...
    function generarCodigo(){
        ...
        foreach ($arreglo as $data){
            ...
            if($data['checks']=='PK'){
                ...
                elseif ($data['nombre']=='obs_dba'){ 
                    //#133 no considera el campo obs_dba por que es solo mantenerse en la base de datos
                    //no lo insertamos en ningun lado
                }                
                else{
    ...
    IF (p_transaccion='".$this->gTabla->getNombreTransaccion()."SEL') THEN
        ...
        BEGIN
            ...
            v_consulta:='SELECT";
                        ...
                        JOIN segu.tusuario usu1 ON usu1.id_usuario = ".$this->gTabla->getAliasLower().".id_usuario_reg
...
***************************************************************************
 HISTORIAL DE MODIFICACIONES:
#ISSUE                FECHA                AUTOR                DESCRIPCION
 #0                '.date("d-m-Y H:i:s").'    '.$_SESSION["_LOGIN"].'             Creacion    
 #
 ***************************************************************************/
';
            } else{
