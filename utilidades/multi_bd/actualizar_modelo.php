<?php
	...
	$dir = '/var/www/html/bja/sis_bja/modelo/';
	$prefijo='MOD';
	$openDir = opendir($dir);
	
	//NOTA: EL USUARIO APACHE DEBE TENER PERMISOS DE LECTURA Y ESCRITURA DE TODA LOS ARCHIVOS PARA PODER PROCESARLOS
	
	while ($archivo = readdir($openDir)){
		$find=strpos($archivo,$prefijo);
		if($find===false){
		} else {
			echo '* '.$archivo.'<br />';
			
			//$archivo = '../../../sis_seguridad/modelo/MODSesion.php';
			//$archivoN = '../../../sis_seguridad/modelo/MODSesionN.php';
			$archivo = $dir.$archivo;
			$archivoN = $archivo.'.nuevo';
