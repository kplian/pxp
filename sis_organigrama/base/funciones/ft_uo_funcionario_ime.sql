...
***********************************/
          ...
          BEGIN
              if ( (select count(*) 
                    from  orga.tuo_funcionario f
                    inner join orga.tuo r  on r.estado_reg=''activo'' and r.cargo_individual=''si''
                    where    f.id_uo=v_parametros.id_uo 
                        and f.estado_reg=v_parametros.estado_reg and
                     f.id_funcionario!=v_parametros.id_funcionario) > 0 ) then
               ...
               if ( v_parametros.estado_reg=''activo'' and
                      ((select count(id_funcionario) from 
                           ...
                           and  id_uo!=v_parametros.id_uo))>0  ) then
