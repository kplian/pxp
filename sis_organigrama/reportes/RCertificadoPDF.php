...
class RCertificadoPDF extends  ReportePDF{
    function Header() {
        ...
        $this->writeHTML($html);
        $cabecera = '<p style="font-family:Century Gothic, serif; font-style:italic;text-align: justify">La suscrita Lic. '.$this->datos[0]['jefa_recursos'].' <b>Jefe de Recursos Humanos</b> de la Empresa Pública Nacional Estratégica "Boliviana de Aviación - BoA", a solicitud '.$this->datos[0]['tipo'].'</p>';
        $this->ln(10);
    ...
    function reporteGeneral(){
        ...
        $viaticos='<p style="font-family:Century Gothic, serif; font-style:italic;text-align: justify">Asimismo a solicitud expresa se informa que '.$gen.' '.$this->datos[0]['tipo_trabajador'].' ha percibido en los últimos tres meses por concepto de viáticos un promedio menusal de '.number_format($this->datos[0]['importe_viatico'],2,",",".").'.- ('.$this->datos[0]['literal_importe_viatico'].' BOLIVIANOS) aclarándo que el <b>Viático</b> es la suma que reconoce la empresa a la persona comisionada, para cubir grastos del viaje.</p><br>';
        if ($this->datos[0]['tipo_certificado'] =='Con viáticos de los últimos tres meses') {
            $this->writeHTML($viaticos);
        }
        $fecha='<p style="font-family:Century Gothic, serif; font-style:italic;text-align: justify">Es cuando se certifica, para fines de derecho que convengan '.$genr.' '.$this->datos[0]['tipo_ge'].'.</p><p style="font-family:Century Gothic, serif; font-style:italic;text-align: justify">Cochabamba '.$this->fechaLiteral($this->datos[0]['fecha_solicitud']).'.</p>';
