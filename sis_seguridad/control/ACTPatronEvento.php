<?php
/***
 Nombre: ACTPatronEvento.php
 Proposito: Clase de Control para recibir los parametros enviados por los archivos
 de la Vista para envio y ejecucion de los metodos del Modelo referidas a la tabla de Patrones de Evento 
 Autor:	Kplian
 Fecha:	01/07/2010
 */
class ACTPatronEvento extends ACTbase{    
	...
	function guardarPatronEvento(){
		...
		if($this->objParam->insertar('id_patron_evento')){
			...
			$this->res=$this->objFunSeguridad->insertarPatronEvento($this->objParam);			
		}
		else{	
			//ejecuta el metodo de modificar persona de la clase MODPersona a travez 
			//de la intefaz objetoFunSeguridad 
			$this->res=$this->objFunSeguridad->modificarPatronEvento($this->objParam);
		}
