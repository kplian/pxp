	...
	if(p_transaccion='SG_ISS_SEL')then
    	...
    	begin
			...
			v_consulta:='select
						...
						inner join segu.tusuario usu1 on usu1.id_usuario = iss.id_usuario_reg
						left join segu.tusuario usu2 on usu2.id_usuario = iss.id_usuario_mod
				        where  ';
...
COST 100;
ALTER FUNCTION "segu"."ft_issues_sel"(integer, integer, character varying, character varying) OWNER TO postgres;
