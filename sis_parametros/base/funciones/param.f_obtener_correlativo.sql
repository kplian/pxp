...
BEGIN
    ...
    */
    ...
    -- raise exception 'par_codigo_documento =% par_codigo_subsistema=%',par_codigo_documento,par_codigo_subsistema;
    ...
    INNER JOIN segu.tsubsistema s on s.id_subsistema = d.id_subsistema and s.codigo = par_codigo_subsistema
	...
	--  #72
    elsif (v_tipo_numeracion = 'simple') then
    	if par_simple is null then
    		raise exception 'La numeracion del documento requiere indicar ID SIMPLE';
        else
	        select d.codigo into v_codigo_depto from param.tdepto d where d.id_depto = par_id_depto;
    	end if;
    else
   ...
   ELSE
    ...
    raise notice '5 simple >>> %',pxp.f_iif(v_tipo_numeracion in ('depto','simple'),par_simple::varchar, 0::varchar)::integer;
 	--  #72
    v_where = '   id_documento='||par_id_documento::varchar ||'
                  ...
                  and simple  '||  pxp.f_iif(v_tipo_numeracion in ('depto','simple'), '= '||par_simple::varchar,'is NULL')||'
	...
	if(v_correlativo is NULL) then
		...
		values (
            ...
            pxp.f_iif((v_tipo_numeracion in ('depto','simple')),par_simple::varchar, NULL)::integer       --#72
            );
    ...
    raise notice 'correlativo % tipo  % gestion % formato % formato_doc %',v_correlativo,v_tipo_numeracion,v_num_gestion,par_formato,v_formato_doc;
	...
	IF(par_formato = 'sin')THEN
  	...
  	ELSEIF par_formato is not null THEN
		...
		v_formula = replace(v_formula,'simple', COALESCE(par_simple::varchar,'')::varchar);		   --#72
     	return  v_formula;
  	ELSE
    	IF(v_formato_doc is not null) THEN
            ...
            v_formula = replace(v_formula,'correlativo', COALESCE(pxp.f_iif(par_digitos_correlativo>0,pxp.f_llenar_ceros(v_correlativo::numeric,par_digitos_correlativo),v_correlativo::varchar),'')::varchar);
            v_formula = replace(v_formula,'codtabla', COALESCE(par_cod_tabla::varchar,'')::varchar);
            v_formula = replace(v_formula,'simple', COALESCE(par_simple,'')::varchar);     --#72
         	return  v_formula;
     	ELSE
        	v_formula='';
          	...
          	ELSEIF v_tipo_numeracion = 'simple' THEN
          		v_formula = COALESCE(v_codigo_depto,'');         --#72
          	END IF;

          	IF  v_periodo_gestion = 'periodo' THEN
          		IF v_tipo_numeracion = 'simple' THEN
            		v_formula = v_formula||'-'||COALESCE(pxp.f_iif(par_digitos_periodo>0,pxp.f_llenar_ceros(v_num_periodo::numeric,par_digitos_periodo),v_num_periodo::varchar),'')||'/'||COALESCE(v_correlativo::varchar,'')||'-'||COALESCE(v_num_gestion::varchar,'');
            	ELSE
          	...
          	ELSE
          		IF v_tipo_numeracion = 'simple' THEN
            		v_formula = v_formula||'-'||COALESCE(pxp.f_iif(par_digitos_correlativo>0,pxp.f_llenar_ceros(v_correlativo::numeric,par_digitos_correlativo),v_correlativo::varchar),'')||'-'||COALESCE(v_num_gestion::varchar,'');
            	ELSE
...
ALTER FUNCTION "param"."f_obtener_correlativo"(par_codigo_documento varchar, par_id integer, par_id_uo integer, par_id_depto integer, par_id_usuario integer, par_codigo_subsistema varchar, par_formato varchar, par_digitos_periodo integer, par_digitos_correlativo integer, par_tabla varchar, par_id_tabla integer, par_cod_tabla varchar, par_id_empresa integer, par_saltar_inicio varchar, par_forzar_inicio varchar, par_simple integer)
  OWNER TO postgres;