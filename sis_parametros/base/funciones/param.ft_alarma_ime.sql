...
BEGIN
	...
	***********************************/
        ...
        ***********************************/
    	...
    	/*********************************
 	#TRANSACCION:  'PM_ALATO_ELI' #124
 	#DESCRIPCION:   Eliminar todo los registro d
 	#AUTOR:		MMV
 	#FECHA:		31/03/2020
	***********************************/

	elsif(p_transaccion='PM_ALATO_ELI')then

		begin

        if (p_administrador) then
        	raise exception 'Eres Admin no puedes eliminar las alertas';
        else

           for v_alertas in (  select al.id_alarma
                               from param.talarma al
                               where al.id_usuario = v_parametros.id_usuario)loop

              --elima las alrmas dependientes
			delete from param.talarma
            where id_alarma_fk = v_alertas.id_alarma;


            --elimina la alrma
			delete from param.talarma
            where id_alarma=v_alertas.id_alarma;
...
ALTER FUNCTION param.ft_alarma_ime (p_administrador integer, p_id_usuario integer, p_tabla varchar, p_transaccion varchar)
  OWNER TO postgres;