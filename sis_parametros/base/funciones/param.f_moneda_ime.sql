  ...
  ***********************************/
    ...
    begin
      ...
      v_id_moneda = param.f_get_moneda_base();
            ...
            --Obtener descripcion
            select moneda, codigo into v_desc_moneda, v_cod_moneda from param.tmoneda where id_moneda = v_id_moneda;
               
            --Definicion de la respuesta
            v_resp = pxp.f_agrega_clave(v_resp,'mensaje','Moneda base encontrada'); 
            v_resp = pxp.f_agrega_clave(v_resp,'id_moneda',v_id_moneda::varchar);
            v_resp = pxp.f_agrega_clave(v_resp,'moneda',v_desc_moneda);
            v_resp = pxp.f_agrega_clave(v_resp,'codigo',v_cod_moneda);
     ...
     /*********************************    
  #TRANSACCION:  'PM_MONTRIAN_GET'
  #DESCRIPCION: Obtener la moneda de Triangulacion
  #AUTOR:     admin
  #FECHA:     
  ***********************************/

  elsif(p_transaccion='PM_MONTRIAN_GET')then

    begin
      --Obtener la moneda base
      v_id_moneda = param.f_get_moneda_triangulacion();
            
            --Obtener descripcion
            select moneda, codigo into v_desc_moneda, v_cod_moneda from param.tmoneda where id_moneda = v_id_moneda;
               
            --Definicion de la respuesta
            v_resp = pxp.f_agrega_clave(v_resp,'mensaje','Moneda triangulacion encontrada'); 
            v_resp = pxp.f_agrega_clave(v_resp,'id_moneda',v_id_moneda::varchar);
            v_resp = pxp.f_agrega_clave(v_resp,'moneda',v_desc_moneda);
            v_resp = pxp.f_agrega_clave(v_resp,'codigo',v_cod_moneda);
