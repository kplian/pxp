...
<<<<<<< Updated upstream
    ...
    ***********************************/
    ...
    #DESCRIPCION:   Obtención de columnas (claves) de una vista para la plantilla de tipo documentos
    #AUTOR:         RCM
    #FECHA:         27/03/2015
    ***********************************/
        ...
        BEGIN
            ...
            if v_parametros.nombre_vista is null THEN
                raise exception 'Debe especificar una Vista existente en la base de datos';
            end if;
    ...
    ***********************************/
    ...
    /*********************************    
    #TRANSACCION:  'WF_TPROTIDOC_SEL'
    #DESCRIPCION:   Obtención de Tipos de Documentos por Tipo de Proceso y Estado
    #AUTOR:         RCM
    #FECHA:         04/04/2015
    ***********************************/

    elsif(p_transaccion='WF_TPROTIDOC_SEL')then
        ...
        BEGIN
           ...
           v_consulta:='select
                        ...
                        tdoc.nombre,
                        tdoc.id_proceso_macro,
                        tdoc.codigo,
                        tdoc.descripcion,
                        tdoc.estado_reg,
                        tdoc.tipo,
                        tdoc.id_tipo_proceso,
                        tdoc.action,
                        tdoc.id_usuario_reg,
                        tdoc.fecha_reg,
                        tdoc.id_usuario_mod,
                        ...
                        tdoc.solo_lectura,
                        tdoc.nombre_vista,
                        tdoc.nombre_archivo_plantilla,
                        tdoc.esquema_vista
                        from wf.ttipo_proceso tp
                        inner join wf.ttipo_estado te
                        on te.id_tipo_proceso = tp.id_tipo_proceso
                        inner join wf.ttipo_documento tdoc
                        on tdoc.id_tipo_proceso = tp.id_tipo_proceso
                        inner join wf.ttipo_documento_estado tdoce
                        on tdoce.id_tipo_documento = tdoc.id_tipo_documento
                        where ';
    ...
    /*********************************    
    #TRANSACCION:  'WF_TPROTIDOC_CONT'
    #DESCRIPCION:   Conteo de registros
    #AUTOR:         RCM
    #FECHA:         04/04/2015
    ***********************************/

    elsif(p_transaccion='WF_TPROTIDOC_CONT')then

        begin
        
            --Sentencia de la consulta de conteo de registros
            v_consulta:='select
                          count(distinct tdoc.id_tipo_documento)
                          from wf.ttipo_proceso tp
                          inner join wf.ttipo_estado te
                          on te.id_tipo_proceso = tp.id_tipo_proceso
                          inner join wf.ttipo_documento tdoc
    ...
    #DESCRIPCION:   Datos de la vista dinamica para la plantilla de documentos
    #AUTOR:         RCM
    #FECHA:         05/04/2015
    ***********************************/
        ...
        BEGIN
            ...
            if v_parametros.esquema_vista is null THEN
                raise exception 'Esquema no definido';
            end if;
            if v_parametros.id_proceso_wf is null THEN
                raise exception 'Proceso WF no definido';
            end if;

            v_consulta:='select * from ' || v_parametros.esquema_vista || '.' || v_parametros.nombre_vista ||
