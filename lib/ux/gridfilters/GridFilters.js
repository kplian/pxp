...
*/
    ...
    //RAC 23/04/2015 se comenta esta funcion para que no memorice los filtros ...
    ...
    reload : function () {
        if (this.local) {
            ...
            //RAC:  7/08/2010
            //regresa a la pagina 1 cuando se aplica el filtro 
            store.lastOptions.params.start = 0;
            store.reload();
            
            console.log(store.lastOptions.params.start)
        }
    },
    ...
    getFilterData : function () {
        ...
        this.filters.each(function (f) {
            if (f.active) {
            	//Rensi Arteaga Copari , console.log(f)
            	//RAC 8/9/10 PARA FILTRO EN COLUMNAS TIPO COMBO
            	//pfiltro define el nombre de la columna por la que se quiere filtrar
            	
                var d = [].concat(f.serialize());
                for (i = 0, len = d.length; i < len; i++) {
                    filters.push({
                        field: f.pfiltro?f.pfiltro:f.dataIndex,
                        data: d[i]
                    });
                }
            }
...
filters[0][data][value]="someValue3"&
    ...
    buildQuery : function (filters) {
        ...
        if (!this.encode){
            ...
            for (i = 0; i < len; i++) {
                f = filters[i];
                /* RAC, 23/09/2015  codificacion de caracteres especiales en filtro */
                if(f.data.type == 'string'){
                	f.data.value = encodeURIComponent(f.data.value)
                }
