...
Ext.ux.AwesomeCombo = {
	...
	/**
	 * @cfg {Boolean} disableSearchButton
	 * Set this parameter to true to hide trigger clear button.
	 * Defaults to false.
	 */
	...
	/**
	 * @cfg {String} trigger2Class (optional)
	 * css clas por search button
	 * Defaults to "x-form-search-trigger".
	 * RAC
	 */
	...
	initComponent: function() {
		 ...
		 * @event clear
		 * RAC 01112014
		 * se dipsra al selecionar el triguer de busqueda
		 * @param {Ext.ux.AwesomeCombo} combo This combo box
		 */
		'clicksearch'
	...
	refreshDisplay: function(forced) {
		forced = forced || false;
		if (this.rendered === false || (forced === false && (this.isExpanded() || this.isSettingValue))) {
			return (false);
		}
		this.generateDisplayText();
		
		if (this.fireEvent('beforedisplayrefresh', this,this.displayNb, this.displayText, this.valueFound) === false) {
			return (false);
		} 
...
Ext.apply(Ext.ux.AwesomeCombo, {
	...
	onTrigger3Click: function() {
		...
		this.validate();
		this.refreshDisplay();
		this.fireEvent('clicksearch', this);
		this.resumeEvents();
		this.fireEvent('displayrefresh', this, this.displayNb, this.displayText, this.valueFound);
	},
	...
	onStoreLoad: function(store, records, options) {
		for (i in records) {
			if (Ext.isObject(records[i])) {
				...
				records[i].set('checked',(this.isChecked(records[i]) ? 'checked' : 'unchecked'));
				if (this.enableMultiSelect !== true) {
					records[i].endEdit();
				}
			}
