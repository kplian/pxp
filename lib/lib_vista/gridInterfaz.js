...
Phx.gridInterfaz=Ext.extend(Phx.baseInterfaz,{
    ...
    setDatTest:function(cmp,name,typeField,vtype,indice){
    	...
    	if((typeField=='ComboBox' || typeField=='ComboRec' || typeField=='TrigguerCombo'  || typeField=='AwesomeCombo')&&  cmp.mode=='remote'){
    	   ...
    	   cmp.store.load({params:{start:0,limit:this.tam_pag}, 
	         callback : function (r) {
	       		if (r.length > 0 ) {	       				
    ...
    ProbarFormNuevo:function(){
    	 //recorre los parametros visible en formulario
    	 console.log(  this.Atributos)
    	   for (var i = 0; i < this.Atributos.length; i++) {
    	   	if(this.Atributos[i].form){
    	  		
    	  		var name = this.Atributos[i].config.name;
    	   		var typeField=this.Atributos[i].type;
    	   		var vtype=this.Componentes[i].vtype;
    	  		
    	  		if(this.Componentes[i].isVisible()){
    	  			
    	  			this.Componentes[i].getEl().dom.click();
    	  			
    	  			if(!this.Componentes[i].disabled && this.Atributos[i].config.inputType!='hidden'){
    	  					
    	  					this.setDatTest(this.Componentes[i],name,typeField,vtype);
    	  			}
    	  		}
    	  	}
    	 }
    	 var self = this;
    	 setTimeout(function(){
    	 	self.window.buttons[0].showMenu();
    	 	self.window.buttons[0].menu.items.items[1].getEl().dom.click();
    	 	self.window.buttons[1].getEl().dom.click();
