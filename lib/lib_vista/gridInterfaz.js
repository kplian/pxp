...
Phx.gridInterfaz=Ext.extend(Phx.baseInterfaz,{
			...
			if(btn=='yes'){
				...
				if(this.idContenedorPadre){
					...
					if(pagMaestreo.tipoInterfaz=='gridInterfaz'){
					  this.colMaestro=pagMaestreo.getColumnasVisibles(true,this.ExtraColumExport);
				    }
				    if(pagMaestreo.tipoInterfaz=='arbInterfaz'){
				      this.colMaestro=pagMaestreo.getColumnasNodo();
				    }
   ...
   onTabToolBarChange: function(tabpanel,tab){
		...
		this.tbar.items.each(function(item, index, length){
			//console.log('xxxx',  item, index, length)
		...
		this.actualizarSegunTab(tabpanel.getActiveTab().title, grupovigente);
	},
	
	actualizarSegunTab: function(title, indice){
   ...
   constructor: function(config){
		...
		console.log('tamaño...', this.gruposBarraTareas.length);
		...
		if(this.gruposBarraTareas.length > 0){
			this.regiones.push(this.tabtbar);
		}
