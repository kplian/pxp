...
Phx.wizardInterfaz=Ext.extend(Phx.baseInterfaz,{ 
	// Componentes:NULL,
	title:'Frm-Interfaz',
	ActSave:'',
	...
	iniciarArrayBotones:function(){
		...
		if(!this.botones){	
			...
			this.arrayBotones.unshift('->')
			this.toolBar= new Ext.Toolbar(
                          {height:30,
                           defaults:{scale:'small',cls:'x-btn-text-icon bmenu'},
                           items:this.arrayBotones});
        }
	...
	constructor: function(config){
    	...
    	this.form = new Ext.form.FormPanel({
			...
			autoDestroy: true,
			tbar: this.tbar,
			buttons:  this.botones?this.arrayBotones: undefined
		});
		// preparamos regiones
		this.prepararRegion();
