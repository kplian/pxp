...
*  *  **************************************************************************
 ...
 HISTORIAL DE MODIFICACIONES:
  ...
  #1			     30-06-2011			RAC							creacion 
  #10   ENDETR       12/02/2019         RCM KPLIAN                  que no filtre centros de costo pr aprobados, adicion de parametro           
 *#85	EndeEtr		 19/11/2019			EGS							Se cambia el root del combo FUNCIONARIOCAR por id_uo_funcionario y se agrega  como campo
 *#87	ETR			 20.11.2019			MZM							Cambio en paginacion de comboRec PERIODO
 *#118	ETR			 16.03.2020			MZM							Inclusion de columna literal en combo PERIODO
...
Ext.form.ComboRec=function(config){
	    	...
	    	if (config.sysorigen) {
			    	...
			    	if(config.origen=='USUARIO'){
			    		...
			    		configini={
				       				...
				       				store: new Ext.data.JsonStore({
				    					...
				    					totalProperty: 'total',
				    					fields: ['id_funcionario','id_uo','codigo','nombre_cargo','desc_funcionario1','email_empresa','id_lugar','id_oficina','lugar_nombre','oficina_nombre','id_uo_funcionario'],//#85
				    					// turn on remote sorting
				    					remoteSort: true,
			    		...
			    		configini=	{
			    			        ...
			    			        tasignacion:false,
					   				...
					   				store: new Ext.data.JsonStore({
										...
										totalProperty: 'total',
										fields: ['id_periodo','periodo','literal'],//#118
										// turn on remote sorting
										remoteSort: true,
					   				...
					   				displayField: 'literal', //#118 - cambio por issue 107 planillas
					   				tpl:'<tpl for="."><div class="x-combo-list-item"><p>{literal}</p></div></tpl>',
				         ...
				         name:'id_clasificador',
			   			 fieldLabel:'Clasificacion',
			   				...
			   				store: new Ext.data.JsonStore({
		    					...
		    					// turn on remote sorting
		    					remoteSort: true,
		    					baseParams:Ext.apply({par_filtro:'codigo#descripcion'},config.baseParams)
				         ...
				         name:'id_centro_costo',
			   			 fieldLabel:'Centro de Costos',
			   				...
			   				store: new Ext.data.JsonStore({
		    					...
		    					remoteSort: true,
		    					//#10 RCM 12/02/2019
		    					baseParams:Ext.apply({tipo_pres:"gasto,administrativo,recurso,ingreso_egreso",par_filtro:'id_centro_costo#codigo_cc#codigo_uo#nombre_uo#nombre_actividad#nombre_programa#nombre_proyecto#nombre_regional#nombre_financiador', aprobado: 'si'},config.baseParams) 
		    	...
		    	if(config.origen=='TIPOCC'){
		    		...
		    		configini={
			    ...
			    else if(config.origen=='PROVCTABANC'){
			    		...
			    		configini = {
			    			 ...
			    			 turl:config.url?config.url:'../../../sis_parametros/vista/proveedor_cta_bancaria/ProveedorCtaBancaria.php',
				   			 ttitle:'Proveedor Cta Bancaria',
				   			   // tconfig:{width:1800,height:500},
				   			 //tdata:{id_proveedor:1},
				   			 tcls:'ProveedorCtaBancaria',
				   			 pid:this.idContenedor,
			    			 name:'id_proveedor_cta_bancaria',
				   			 fieldLabel:'Proveedor Cta Bancaria',
				   				allowBlank:false,
				   				emptyText:'Proveedor Cta Bancaria...',
				   				store: new Ext.data.JsonStore({
									//url: config.url?config.url:'../../sis_tesoreria/control/CuentaBancaria/listarCuentaBancariaBeneficiario',
									url: config.url?config.url:'../../sis_parametros/control/ProveedorCtaBancaria/listarProveedorCtaBancaria',
									...
									totalProperty: 'total',
									fields: ['id_proveedor_cta_bancaria','nro_cuenta','banco_beneficiario','nombre','desc_cuenta','id_banco_beneficiario','desc_cuenta'],
									// turn on remote sorting
									remoteSort: true,
									baseParams: Ext.apply({par_filtro:'pctaban.nro_cuenta#instben.nombre'},config.baseParams)
								}),
				   				valueField: 'id_proveedor_cta_bancaria',
				   				displayField: 'desc_cuenta',

				   				tpl:'<tpl for="."><div class="x-combo-list-item"><p>{banco_beneficiario}</p><p>Cta:{nro_cuenta}</p> </div></tpl>',
