...
Autor:					Rensi Arteaga Copari
 ...
 #0            27 - 09 - 11      RAC              Creacion
 #28           27/06/2019        RAC              Exportar a PDF en hoja horizontal horizontal
 #65           19/09/2019        RAC              corrección del valor por defecto de tiempo de espera agotado
 #140          12/05/2020        RAC              nueva ufncion getIndAtributoByDisplayField
**********************************************************
*/
...
Phx.baseInterfaz = Ext.extend(Ext.util.Observable, {
   ...
   getBoton: function(id) {
    ...
    //#140
    getIndAtributoByDisplayField: function(name) {
        for (var m in this.Atributos) {
            if (this.Atributos[m].config && this.Atributos[m].config.gdisplayField && this.Atributos[m].config.gdisplayField === name) {
                return m;
            }
            if (this.Atributos[m].config && this.Atributos[m].config.name === name) {
                return m;
            }
        }
        return undefined;
        ...
        if (this.bexcel) {
            cbuttons.push({
                ...
                menu: [{
                    ...
                    text: 'PDF Horizontal',  //#28 add pdf horizontal
                    iconCls: 'bpdf',
                    argument: {
                        'news': true,
                        def: 'pdf_h'
 ...
 */
    ...
    DisableSelect: function(n) {
        ...
        if (this.west) {
            ...
            if (this.tabwest && this.TabPanelWest && this.TabPanelWest.getActiveTab()) {
        ...
        if (this.south) {
            ...
            if (this.tabsouth && this.TabPanelSouth && this.TabPanelSouth.getActiveTab()) {
        ...
        if (this.east) {
            ...
            if (this.tabeast && this.TabPanelEast && this.TabPanelEast.getActiveTab()) {
 ...
 */
   ...
   validarTarjeta:function(){
    ...
    constructor: function(config) {
    	...
    	if(!me.timeout){
      		me.timeout = Phx.CP.config_ini.timeout; //#65 carga valor por  defecto para tiempo de espera
      	}
		...
		//RCM 05/02/2013: función para agregar componentes en interfases heredadas
