 ...
 */
    ...
    loadValoresIniciales: function() {
        ...
        this.getValidComponente(0).focus(true,100);
 ...
 * mas eficiete que utilizar mostrarComponente o ocultarComponente
 ...
 * @autor Rensi Arteaga Copari
 * omuestra todos los componentes de un grupo
 * @param {Ext.integer} idGrupo  
 * 
 */
 ...
 * @function setFormSize
 * @autor Rensi Arteaga Copari
 * Calula la altura  en valor integer a parti de un porcentaje 
 * y el panel especificado de referencia
 * si manda un integerretorna el mismo valos sin cambiar
    ...
    getValidComponente:function(h){
    	...
    	for(h;h<=this.Componentes.length;h++){
        	...
        	if(this.Componentes[h] && !this.Componentes[h].disabled && this.Componentes[h].inputType!='hidden'){
        		console.log('visible',this.Componentes[h].isVisible());
        		console.log('cmp',h,this.Componentes[h].name)
        		return this.Componentes[h];
        		break;
        	}
        	
        		//console.log('cmp ..no incluido..',h,this.Componentes[h].name)
        }
        return undefined;
    },
