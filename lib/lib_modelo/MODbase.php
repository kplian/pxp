...
class MODbase extends driver
{
 ...
 /**
    ...
    //RAC november 2023
    function doRequestPxpNd($methodIn, $service, $returnType = 'JSON') {
        $params = $this->objParam->getArregloParametros();

        //$this->arregloFiles=$this->aParam->getArregloFiles();

        //var_dump($this->arregloFiles);
        //$data =  pg_escape_bytea(base64_encode(file_get_contents($this->arregloFiles[$valor]['tmp_name'])));

        $method = strtoupper($methodIn);
        $curl = curl_init();
        $pxpndSecret =  $_SESSION['pxpndSecret'];
        if ($method === 'POST' || $method === 'DELETE' || $method === 'PATCH') {
            if (count($this->arregloFiles) > 0) {

                $jsonData = $params;
                foreach ($this->arregloFiles as $index => $file) {

                    // Obtener el nombre de la primera llave
                    $firstKey = key($this->arregloFiles);

                    //echo ">>>>>>>>>>>>>> ".$firstKey." ===>     ".$file['tmp_name']." <=====";

                    $filePath = realpath($file['tmp_name']);

                    if (!is_readable($filePath)) {
                        // Manejar el error, por ejemplo, lanzar una excepción o imprimir un mensaje de error.
                        die("No se puede leer el archivo temporal: $filePath");
                    }
                   ...
                   // Mapeo de extensiones a tipos MIME
                    $mimeTypes = [
                        ...
                        'gif' => 'image/gif',
                        'zip' => 'application/zip',
                        // Agregar más extensiones y tipos MIME según sea necesario
                    ];

                    // Establecer el tipo MIME basándose en la extensión del archivo
                    $mimeType = isset($mimeTypes[$fileExtension]) ? $mimeTypes[$fileExtension] : 'application/octet-stream';


                    $jsonData[$firstKey] = curl_file_create(
                        $filePath,
                        $mimeType,
                        basename($file['name'])
                    );

//                    $jsonData['file[' . $firstKey . ']'] = curl_file_create(
//                        realpath($file['tmp_name']),
//                        mime_content_type($file['type']),
//                        basename($file['name'])
//                    );
                }
               // unset($jsonData['_FILES_PATH_']);
                $header = array(
                    'Authorization: ' . $pxpndSecret
            ...
            curl_setopt_array($curl, array(
                CURLOPT_URL => $_SESSION['pxpndUrl'] . $service,
            ...
            curl_setopt_array($curl, array(
                CURLOPT_URL => $_SESSION['pxpndUrl'] . $service . '?' . $urlParams,
                CURLOPT_RETURNTRANSFER => true,
                CURLOPT_ENCODING => '',
                CURLOPT_MAXREDIRS => 10,
                CURLOPT_TIMEOUT => 0,
