...
class RSA{
    ...
    */
    public function generate_keys ($p, $q, $show_debug=0){
    	//p: 961787521   q: 961775917
    	//echo "p: $p   q: $q";exit;
    	//rcm: modificaciones para no utilizar bcmul por lo pronto
      	$n = bcmul($p, $q);
      	//$n = 925024074968931757;
      
      	//m (we need it to calculate D and E) 
      	$m = bcmul(bcsub($p, 1), bcsub($q, 1));
      	//$m = 925024073045368320;
    ...
    */
    private function findE($m){
    	//rcm: se define como constante el valor del parametro
    	//$m=925024073045368320;
    ...
    public function encrypt ($m, $e, $n, $s=3) {
        ...
        for($i=0; $i<$packets; $i++){
            ...
            for($j=0; $j<$s; $j++){
                ...
                //convierte la cadena a encriptar en un numero y lo cancatenando en el ciclo for
            	$code = bcadd($code, bcmul(ord($packet[$j]), bcpow('256',$j)));
                
                
            }
			//eleva el numero obtenido a e y saca el modulo n
